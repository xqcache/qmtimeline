...
void QmTimelineItemModel::setRowHidden(int row, bool hidden)
{
    if (hidden) {
...
bool QmTimelineItemModel::isRowHidden(int row) const
{
...
{
    if (locked) {
        d_->locked_rows.emplace(type);
    } else {
        d_->locked_rows.erase(type);
    }
    setDirty();
}

bool QmTimelineItemModel::isRowLocked(int row) const
{
